...
Skills for Claude Code; five of them also install as Codex CLI skills.
...
| `design-canon` | Navigation contract for a long-form design manual — read the relevant profile for a structural decision instead of dumping the whole document. Bundles one profile, `pragmatic-programmer.md` (DRY, orthogonality, tracer bullets, prototypes). |
...
Codex CLI gets `greenfield`, `brownfield`, `handoff`, `eval-discipline`, and `crap-score` —
the same folders, since Codex reads the same `SKILL.md` format. (Codex dropped custom
prompts; the old `codex/prompts/` mirrors are gone.) Where a skill mentions subagents,
Codex runs that step itself.
...
Copies `claude/skills/*` to `~/.claude/skills/` and the five Codex-ready skills to
`~/.codex/skills/`. Idempotent — re-run to update; same-named skills are replaced,
unrelated ones untouched.

Into one project instead of globally:

```sh
./install.sh --project    # copies to ./.claude/skills (Codex skipped)
```
...
cp -R claude/skills/handoff ~/.codex/skills/
```
...
**Codex CLI.** Skills in `~/.codex/skills/` work the same way: mention one with `$handoff`,
or the model picks it from the description.
